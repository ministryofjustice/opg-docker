<?php
    ...
    /*****
     *
     * Plugin name:     OPG Recently Updated Pages
     * Description:     OPG Specific fork to bring out title & first line from pages
     *
     *                  Purpose of this plugin is to display the list of pages (and optionally posts) on
    ...
    function opg_recently_updated_pages() {
        register_widget('OPG_Recently_Updated_Pages');
    }
    
    // Class Recently Updated Pages is extending WP_Widget class
    class OPG_Recently_Updated_Pages extends WP_Widget {
        function OPG_Recently_Updated_Pages() {
            $widgetSettings     = array (
                                        'classname'     => 'OPG_Recently_Updated_Pages',
            ...
            $controlSettings    = array (
                                        'width'         => 300,
                                        'height'        => 400,
                                        'id_base'       => 'opg_recently_updated_pages'
                                        );
                                        
            $this->WP_Widget('opg_recently_updated_pages', 'OPG Recently Updated Pages', $widgetSettings, $controlSettings);
        }
        ...
        function widget($args, $instance) {
	        ...
	        $withFirstLine      = apply_filters('scDateFormat', $instance['withFirstLine']);
                ...
                echo "<ul>";
                    foreach ($pageList as $obj) {
                        echo "<li class='page_item page-item-{$obj->ID}'>";
                            if ($displayDate == 1 && $withFirstLine == 1) {
                                echo '<p class=\"date\">' . date($dateFormat, strtotime($obj->post_modified)) . '</p>';
                            }
                            echo "<h3><a href='{$obj->uri}' title='{$obj->post_title}'>
                                {$obj->post_title}
                            </a></h3>";
                            if (strlen($obj->post_content) > 128) {
                                $limitedStr = substr($obj->post_content, 0, 128);
                                $limitedStr .= '...';
                            } else {
                                $limitedStr = $obj->post_content;
                            }
                            echo '<p>' . filter_var($limitedStr, FILTER_SANITIZE_STRING) . '</p>';
                            if ($displayDate == 1 && $withFirstLine != 1) {
...
<?php
        ...
        function getListOfPages($totalPagesToShow, $showListWithPosts) {
            ...
            $sql            = "SELECT ID, post_title, post_modified, post_content FROM
                                {$wpdb->posts} WHERE
    ...
    add_action('widgets_init', 'opg_recently_updated_pages');
